...
To make content available for serving from the host filesystem:
```bash
docker run --rm -v /some/host/dir:/data nlnetlabs/krill-sync:v0.2.0-rc1 -v https://<your.fqdn>/notification.xml
```

Or to make content available for serving from a Docker volume shared with other Docker containers:
