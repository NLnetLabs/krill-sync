    ...
    if let Some(new_etag) = new_etag {
        if let Some(prev_etag) = prev_etag {
            if prev_etag == new_etag {
                warn!("RRDP notification file was re-served with an \
        ...
        warn!("RRDP notification file was served without an ETag \
...
fn should_notification_file_be_processed(
    ...
    opt: &Opt,
    state: &State,
    new_state: &State) -> Result<bool>
{
    match (state.notify_serial, notify.serial, &state.notify_etag, &new_state.notify_etag) {
        (prev_serial, new_serial, _, _) if prev_serial < new_serial => {
            Ok(true)
        },
        (prev_serial, new_serial, Some(prev_etag), Some(new_etag)) if prev_etag != new_etag => {
            warn!("RRDP notification file is unchanged (by serial \
                    check: {} == {}) but IS changed by ETag check: \
                    {} != {}). Processing anyway",
                prev_serial, new_serial, prev_etag, new_etag);
            Ok(true)
        },
        (prev_serial, new_serial, _, _) if new_serial < prev_serial => {
            Err(
                anyhow!("RDDP notification file serial is older than the last \
                         seen serial: {} < {}, exiting.",
                    new_serial, prev_serial)
            )
        },
        _ if opt.force_update => {
            Ok(true)
        },
...
fn try_main() -> Result<()> {
    ...
    // ============================================
    let mut raw_notification_file = match rrdp::download_raw_rrdp_notification_file(
        &opt, &state, &rrdp_http_client)?
    {
        None => {
            info!("RRDP notification file is unchanged (by ETag check), exiting.");
            return Ok(());
        },
    ...
    warn_about_unexpected_etag_values(
        &state.notify_etag,
        &new_state.notify_etag);

    let mut notify = rrdp::parse_notification_file(&raw_notification_file)?;
    ...
    let (raw_snapshot, generated) = rrdp::get_snapshot(
        state_loaded, &state, &opt, &mut notify, &rrdp_http_client,
    ...
    if generated {
        ...
        raw_notification_file = rrdp::update_notification_hash(
            raw_notification_file, &mut notify, rrdp::calc_hash(&raw_snapshot))?;
    }
    ...
    if is_rrdp_format_enabled(&opt) {
        ...
        let snapshot_dir_path = opt.rrdp_dir.join(
            rrdp::make_delta_dir_path(&notify, notify.serial)?);
