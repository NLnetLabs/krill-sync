...
pub fn build_repo_from_rrdp_snapshot(rrdp_state: &RrdpState, config: &Config) -> Result<()> {
    ...
    if config.rsync_dir_use_symlinks() {
        info!(
            ...
            rsync_dir_for_snapshot_name, rsync_dir
        );
        ...
        std::os::unix::fs::symlink(rsync_dir_for_snapshot_name, &tmp_name).with_context(|| {
            format!(
        ...
        if current_path.exists() {
            ...
            if let Ok(prev_rev_id_bytes) = file_ops::read_file(&current_revision_id_file) {
                ...
                if let Ok(prev_rev_id_str) = std::str::from_utf8(&prev_rev_id_bytes) {
                    ...
                    info!(
                        "Rename current rsync dir from '{:?}' to '{:?}'",
                        current_path, prev_rev_id_path
                    );
                    std::fs::rename(&current_path, &prev_rev_id_path).with_context(|| {
                        format!(
        ...
        info!(
            "Preserving revision path for future reference in '{:?}'",
            current_revision_id_file
        );
        file_ops::write_buf(
            &current_revision_id_file,
            out_path.to_string_lossy().as_bytes(),
        )
        .with_context(|| {
            format!(
                "Could not write current revision info id file '{:?}'",
                current_revision_id_file
            )
        })?;

        info!(
            "Rename new rsync dir from '{:?}' to '{:?}'",
            out_path, current_path
        );
        std::fs::rename(&out_path, &current_path).with_context(|| {
            format!(
